...
// Command ids - SMPP v3.4 - 5.1.2.1 page 110-111
  ...
  /**
   * Send one SMS from SMSC to Complainant. Can be executed only after bindTransmitter() call.
   * @return true on succesfull send, false if error encountered
   */
  function sendToComplainant($from, $complainant, $message){
    if (strlen($from)>20 || strlen($complainant)>20 || strlen($message)>160)return false;
    if($this->state!="bind_tx")return false;
    
    //TON
    $this->sms_source_addr_ton = $this->setTon($from);
    $this->sms_dest_addr_ton = $this->setTon($complainant);

    //NPI
    $this->sms_source_addr_npi = $this->setNPI($from);
    $this->sms_dest_addr_npi = $this->setNPI($complainant);

    $pdu = pack('a1cca'.(strlen($from)+1).'cca'.(strlen($complainant)+1).'ccca1a1ccccca'.(strlen($message)+1),
  ...
  /**
   * Send one SMS from SMSC to Respondent. Can be executed only after bindTransmitter() call.
   * @return true on succesfull send, false if error encountered
   */
  function sendToRespondent($from, $respondent, $message){
    if (strlen($from)>20 || strlen($respondent)>20 || strlen($message)>160)return false;
    if($this->state!="bind_tx")return false;
    
    //TON
    $this->sms_source_addr_ton = $this->setTon($from);
    $this->sms_dest_addr_ton = $this->setTon($respondent);

    //NPI
    $this->sms_source_addr_npi = $this->setNPI($from);
    $this->sms_dest_addr_npi = $this->setNPI($respondent);

    $pdu = pack('a1cca'.(strlen($from)+1).'cca'.(strlen($respondent)+1).'ccca1a1ccccca'.(strlen($message)+1),
      $this->sms_service_type,
      $this->sms_source_addr_ton,
      $this->sms_source_addr_npi,
      $from,//source_addr
