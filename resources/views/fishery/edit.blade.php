...
<div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label">Region</label><select name="region_id" id="regionSelect" class="form-select"><option value="">Select</option>@foreach($regions as $r)<option value="{{ $r->id }}" {{ ($fisherman->region_id == $r->id) ? 'selected' : '' }}>{{ $r->region }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label">District</label><select name="district_id" id="districtSelect" class="form-select"><option value="">Select</option>@foreach($districts as $d)<option value="{{ $d->id }}" {{ ($fisherman->district_id == $d->id) ? 'selected' : '' }}>{{ $d->district }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label">Ward</label><select name="ward_id" id="wardSelect" class="form-select"><option value="">Select</option>@foreach($wards as $w)<option value="{{ $w->id }}" {{ ($fisherman->ward_id == $w->id) ? 'selected' : '' }}>{{ $w->ward }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label">Village/Street</label><select name="village_id" id="villageSelect" class="form-select"><option value="">Select</option>@foreach($villages as $v)<option value="{{ $v->id }}" {{ ($fisherman->village_id == $v->id) ? 'selected' : '' }}>{{ $v->village }}</option>@endforeach</select></div>
